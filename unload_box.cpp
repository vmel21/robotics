...
int main(int argc, char** argv) {
	...
	//loop through all the parts, looking for the "name" of the defective part
	for (int i = 0; i< orphan_models_wrt_world.size(); i++) {
		if (abs(orphan_models_wrt_world[i].pose.position.z - current_part.pose.pose.position.z) < 0.01 && abs(orphan_models_wrt_world[i].pose.position.y - 				current_part.pose.pose.position.y) < 0.01 && abs(orphan_models_wrt_world[i].pose.position.x - current_part.pose.pose.position.x) < 0.01) {
			current_part.name = orphan_models_wrt_world[i].type;
		}

	}
	
	ROS_INFO("Now, attempting to remove the bad part");

	//remove the bad part from the box
	...
	ROS_INFO("Now attempting to remove the remaining parts");
	//now discard the remaining parts
    for (int i=0;i<nparts;i++) {
